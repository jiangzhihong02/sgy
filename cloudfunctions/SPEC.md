...
- `getRules`：下发面向用户规则面板 `{ timeline, preview, creditText, privacyText, creditTable, creditFooter, privacySections, limits:{imgMax,…} }`——文案与数值唯一来源 `rides/rules.js rulePayload()`（同文件同常量，改数值自动带出文案）；`creditTable`/`privacySections` 为可视化面板的结构化数据（信用分表格 / 隐私分节）；客户端 `sgy/utils/rulesText.js` 快照兜底。
