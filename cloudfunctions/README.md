...
| `rides` | **唯一业务函数**（原 `user` 已并入删除）：拼车局 create/list/my/detail/join/leave/cancel/checkin/respondPoll/sendMessage/… + 用户档案·信用·管理员复核 login/me/register/adminPending/resolveReport/banUser/adminSetGender（`account.js`） | 小程序调用 |
...
3. **上传云函数**：分别右键 `cloudfunctions/routeInit`、`rides`、`rideSweep` →「上传并部署：云端安装依赖」。`rides` 已含原 `user` 能力，请在云开发控制台**删除旧的 `user` 函数**。`rideSweep` 的定时触发器在 `config.json`，上传后可在「云开发控制台 → 云函数 → rideSweep → 触发器」确认每分钟一次。
