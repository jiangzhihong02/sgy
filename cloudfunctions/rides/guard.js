...
//
// 判定顺序**统一**为：成员 → 状态 → 窗口（2026-09-10 收拢；此前各 action 顺序不一）。
// 错误码归一到三类：NOT_IN（不是成员）/ BAD_STATE（状态不允许）/ TOO_LATE（过了可操作时间）。
// 动作**特有**的前置条件（如 respondPoll 的"当前没有人数确认"）不进这里，
// 由动作自己在 fn 里返回——避免为个别动作把 interface 撑大（见 docs/regression-checklist 之外的 C2 记录）。
